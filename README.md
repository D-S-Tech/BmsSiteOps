...
> **Status (May 2026): Sprints 0-10 closed. Platform feature-complete: Site Q&A with RAG + MCP server, 3-layer MCP defense, pgvector vector search, gated live-integration harness, smoke + mcp-smoke + cert-status validators, MCP audit log, backup/restore. Sprint 10 closed the last operator-side gap: the production Postgres now ships with TimescaleDB + pgvector pre-installed (image swap + init script) and `make pg-ext-check` verifies on every deploy. **392 unit tests** + 5 live integration suites; 5 CI jobs.**
...
<tr><td><b>9</b></td><td>🟢 Done</td><td>Operational hardening — <code>make cert-status</code> Caddy TLS expiry monitor (cron-friendly exit codes) · MCP tool-call audit log: worker stamps <code>X-MCP-Tool</code> header, Laravel <code>mcp.audit</code> middleware writes <code>mcp_audit_entries</code> per call, nightly <code>audit:prune-mcp</code> retention · <code>make backup</code> + <code>make backup-restore</code> (postgres custom dump + caddy_data + api_storage tarballs, optional S3 upload, 30-day local retention)</td></tr>
<tr><td><b>10</b></td><td>🟢 Done</td><td>Postgres extensions baseline — production postgres now ships with TimescaleDB + pgvector pre-installed (image swap to <code>timescale/timescaledb-ha:pg16</code>, both ext-s built in) · idempotent init script <code>infra/postgres/init/01-enable-extensions.sql</code> runs on first DB start · <code>make pg-ext-check</code> verifier prints extension/version/schema for each enabled ext · Sprint 3 hypertable conversion and Sprint 8.2 pgvector migrations now activate automatically without per-host apt-install</td></tr>
</tbody>
</table>
...
> **pgvector-deployment note:** after Sprint 10, activating the production PG optimization is just `make prod-migrate` — the postgres image (`timescale/timescaledb-ha:pg16`) ships with pgvector pre-installed and the init script enables the extension on first DB start. The three Sprint 8.2 migrations no-op silently on SQLite/MySQL, so re-running CI never touches them. The Sprint 7.2 JSON-text embedding column stays — pgvector reads from a separate `embedding_pg` mirror column synced by a trigger, so re-embedding existing chunks is automatic. Verify with `make pg-ext-check`.
...
- [x] 9.3 — `make backup` + `make backup-restore` (postgres + volume snapshots): `infra/scripts/backup.sh` captures three artifacts per run into a timestamped subdirectory of `$BACKUP_DIR` (default `/var/lib/bmssiteops/backups`) — postgres custom-format dump via `pg_dump` against the running container, `caddy_data` volume tarball (preserves Let's Encrypt certs + ACME state), and `api_storage` volume tarball (Laravel uploads + logs). Optional fourth step uploads the directory to S3 if `BACKUP_S3_BUCKET` is set (uses host `aws` CLI). Local retention via `BACKUP_RETENTION_DAYS` (default 30). Companion `backup-restore.sh` refuses to run while app containers are up (would corrupt volumes), prompts for confirmation, and walks the inverse pipeline. Distinct exit codes (1-5) so cron + alerting hooks know which artifact failed

### Sprint 10 detail

- [x] 10.1 — Postgres extensions baseline: production postgres image swapped from `postgres:16-alpine` to `timescale/timescaledb-ha:pg16` (Timescale's official image bundles both TimescaleDB and pgvector at the OS level, so per-host apt-install is no longer needed). New init script `infra/postgres/init/01-enable-extensions.sql` runs once on first DB start via postgres's `/docker-entrypoint-initdb.d/` convention, idempotently enabling both extensions inside the BmsSiteOps database. New `make pg-ext-check` (`infra/scripts/pg-ext-check.sh`) verifies both extensions are present, prints name/version/schema for each, plus a hypertable inventory (Sprint 3) and vector-column inventory (Sprint 8.2). Cron-friendly exit codes: 0=ok, 1=missing tool, 2=postgres down, 3=at least one extension missing. Sprint 3 hypertable conversion and Sprint 8.2 pgvector migrations now activate cleanly without any extra operator step

> **Postgres image note:** `timescale/timescaledb-ha:pg16` is debian-based and larger than `postgres:16-alpine` (~500 MB vs ~250 MB on disk). The trade-off is intentional — no Dockerfile to maintain, no apt-install per host, no version drift between operators. The HA suffix means the image also ships with patroni + pgbackrest, but those run only when explicitly configured, so a vanilla single-node BmsSiteOps deploy doesn't pay for them at runtime. To roll back, change the `image:` line in `infra/compose/docker-compose.prod.yml` back to `postgres:16-alpine` — the init script becomes a no-op when the extension binaries don't exist at the OS level, and operators would need to install pgvector + timescaledb manually as in the original Sprint 3/8.2 notes.
